...
class VoiceClonePromptItem:
    ...
    def save_to_file(self, path: str) -> None:
        """Save this item to a .pt file."""
        torch.save({
            "voice_clone_prompt_item": self.to_dict(),
            "version": 1,
        }, path)

    @classmethod
    def load_from_file(cls, path: str, device: Optional[torch.device] = None) -> "VoiceClonePromptItem":
        """Load a VoiceClonePromptItem from a .pt file."""
        data = torch.load(path, map_location="cpu", weights_only=True)
        item_dict = data.get("voice_clone_prompt_item", data)
        return cls.from_dict(item_dict, device=device)
...
class Qwen3TTSModel:
    ...
    # ======================== Voice Vector Save / Load ========================

    def save_voice_vector(
        self,
        prompt_items: Union[VoiceClonePromptItem, List[VoiceClonePromptItem]],
        output_path: str,
    ) -> str:
        """
        Save voice clone prompt item(s) to a .pt file for later reuse.

        This allows you to extract the speaker embedding (and optionally speech codes)
        once from reference audio, save it, and then use the saved vector directly
        in future TTS generation without needing the original audio file.

        Args:
            prompt_items:
                Single VoiceClonePromptItem or list of them (from create_voice_clone_prompt).
            output_path:
                Path to save the .pt file. Will be created if it doesn't exist.

        Returns:
            The absolute path where the file was saved.

        Example:
            >>> items = tts.create_voice_clone_prompt(ref_audio="ref.wav", ref_text="...", x_vector_only_mode=True)
            >>> tts.save_voice_vector(items, "my_voice.pt")
            # Later...
            >>> loaded_items = tts.load_voice_vector("my_voice.pt")
            >>> wavs, sr = tts.generate_voice_clone(text="Hello world", voice_clone_prompt=loaded_items)
        """
        ...
        save_data = {
            ...
            "tokenizer_type": self.model.tokenizer_type,
            "items": [it.to_dict() for it in cpu_items],
        }
    ...
    def load_voice_vector(
        self,
        path: str,
        device: Optional[Union[str, torch.device]] = None,
    ) -> List[VoiceClonePromptItem]:
        """
        Load voice clone prompt item(s) from a .pt file.

        Args:
            path:
                Path to the .pt file saved by save_voice_vector().
            device:
                Target device for loaded tensors. If None, uses model's device.

        Returns:
            List[VoiceClonePromptItem]: Loaded prompt items ready for generate_voice_clone().
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(f"Voice vector file not found: {path}")

        data = torch.load(path, map_location="cpu", weights_only=True)

        # Handle both old (single item) and new (dict with 'items') formats
        if "items" in data:
            items_dict_list = data["items"]
        elif "voice_clone_prompt_item" in data:
            items_dict_list = [data["voice_clone_prompt_item"]]
        else:
            # Legacy: assume it's a single item dict
            items_dict_list = [data]

        target_device = device if device is not None else self.device

        items = []
        for item_dict in items_dict_list:
            item = VoiceClonePromptItem.from_dict(item_dict, device=target_device)
            items.append(item)
